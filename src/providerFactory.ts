...
import { PrivateGiteaProvider } from "./providers/PrivateGiteaProvider"
...
export function createClient(data: PublishConfiguration | AllPublishOptions, updater: AppUpdater, runtimeOptions: ProviderRuntimeOptions): Provider<any> {
  ...
  switch (provider) {
    case "gitea": {
      const giteaOptions = data as GiteaOptions
      const token = (giteaOptions.private ? process.env["GH_TOKEN"] || process.env["GITHUB_TOKEN"] : null) || giteaOptions.token
      if (token == null) {
        return new GiteaProvider(giteaOptions, updater, runtimeOptions)
      } else {
        return new PrivateGiteaProvider(giteaOptions, updater, token, runtimeOptions)
      }
    }
