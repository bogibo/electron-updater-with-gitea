import { CancellationToken, GiteaOptions, githubUrl, HttpError, newError, parseXml, ReleaseNoteInfo, UpdateInfo, XElement } from "builder-util-runtime"
...
export abstract class BaseGitHubProvider<T extends UpdateInfo> extends Provider<T> {
  ...
  protected constructor(protected readonly options: GiteaOptions, defaultHost: string, runtimeOptions: ProviderRuntimeOptions) {
    super({
      ...runtimeOptions,
...
export class GiteaProvider extends BaseGitHubProvider<GithubUpdateInfo> {
  constructor(protected readonly options: GiteaOptions, private readonly updater: AppUpdater, runtimeOptions: ProviderRuntimeOptions) {
  ...
  async getLatestVersion(): Promise<GithubUpdateInfo> {
    ...
    if (tag == null) {
      throw newError(`No published versions`, "ERR_UPDATER_NO_PUBLISHED_VERSIONS")
    }
  ...
  private async getLatestTagName(cancellationToken: CancellationToken): Promise<string | null> {
    ...
    try {
      ...
      throw newError(`Unable to find latest version on (${url}), please ensure a production release exists: ${e.stack || e.message}`, "ERR_UPDATER_LATEST_VERSION_NOT_FOUND")
    }
  }
